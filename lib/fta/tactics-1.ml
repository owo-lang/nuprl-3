...
let MemberIntro =
  ...
  Try
	...
	let T'.tl' = map_on_some 
	       	     (\x. let (),n = no_extraction_compute x in
			  if n=0 then fail
  ...
  (\ p .
	...
	let Trivial = Hypothesis ORELSE (Refine equality) in
	...
	if is_apply_term t & is_set_term T then
		Trivial
		ORELSE
