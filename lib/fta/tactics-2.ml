...
let RepeatFunctionElimFor i n instantiation_list =
  ...
  letrec Aux j n instantiation_list =
	...
	(let (),A = destruct_hyp j p in
	 ...
	 if x=`NIL` then
		...
		Refine (function_elim j (hd instantiation_list) `NIL`)
		THEN
		(if i=j then Idtac else Thinning [j])
		THENL
		[Idtac
		;\p. Aux (number_of_hyps p) (n-1) (tl instantiation_list) p
		]
	)
	p     
     )				in
...
let RepeatFunctionElim i instantiation_list  =
  ...
  letrec Aux j instantiation_list =
	...
	(let (),A = destruct_hyp j p in
	 ...
	 if x=`NIL` then
		...
		THENL
		[Idtac
		;\p. Aux (number_of_hyps p) (tl instantiation_list) p
		]
	)
	p     
     )				in
...
let RepeatFunctionNotElim i instantiation_list  =
...
%  Match the conclusion against part of the conclusion of the implication in
   ...
   Use the term list for the terms required to elim the implication.
%
let BackThruHypUsing i term_list p =    	    
	let T = type_of_hyp i p in
	let C = concl p   in
	let inst_list = match_part_using (concl p) T term_list  in
	( Progress (RepeatAndElim i THEN Hypothesis)
	  ORELSE
	  (  RepeatFunctionNotElim i inst_list
	     THEN
	     (\ p .
		(if C = concl p then
...
let InstantiateLemma name term_list =
	...
	THENL
	[Refine (lemma name `NIL`) 
         THEN ApplyToLastHyp (\i. RepeatFunctionElim i term_list)
	;Idtac
	]
