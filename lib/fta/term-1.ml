...
% For t of the form x1:A1 -> . . . -> xm:Am -> (B1&...&Bn), match s 
  against the Bi's.  Return a list of instantiating terms, in order,
  for the non-nil xi's, obtained either from the first successful match,
  or from the supplied list if the match did not determine an instantiating
  term, (so the number of non-nil xi's must be equal to the number of
  variables bound by the match plus the length of the term list).
%
