...
% The following definitions are defunct.  They had problems.  They tried to use
  both the bindings resulting from a match, and supplied terms.  Which to use
  for a certain elim was determined by the all id.  Unfortunately, this broke
  when substitution caused alpha conversion.
%
%
...
let RepeatAtomicNotFunctionElim i instantiation_list =
...
(       Progress (RepeatAndElim i THEN Hypothesis)
        ...
        (       (if is_not_term C then RepeatAtomicNotFunctionElim else RepeatFunctionElim)
                        i (map snd match_bindings @ inst_list)
                THEN
                (\ p .
                        (if C = concl p then
...
let Backchain = BackchainWith Idtac ;;
